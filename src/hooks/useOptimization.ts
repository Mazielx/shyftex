 ...
 *   parse list -> match products -> fetch stores/prices -> optimize -> results
 *
...
export function useOptimization(): UseOptimizationReturn {
  ...
  const runOptimization = useCallback(
    async (rawInput: string) => {
      ...
      try {
        // 1. Parse the raw shopping list (this also matches products)
        ...
        // 5. Fetch prices, promotions, and inventory for each store
        ...
        // 6. Build optimization input
        const vehicle = useSettingsStore.getState().vehicle;
        const input: OptimizationInput = {
          listId: useListStore.getState().currentList?.id ?? '',
          userId: 'user_1',
          items: currentItems,
          stores,
          prices: pricesMap,
        ...
        // 7. Run the optimization engine
        const result = optimizationEngine.optimize(input);

        // 8. Also update the store so other components can access the plans
